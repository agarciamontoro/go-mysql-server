...
var fixtures = map[string]sql.Node{
	...
	`SELECT 0x01AF`: plan.NewProject(
		[]sql.Expression{
			expression.NewLiteral(int32(431), sql.Int32),
		},
	...
	`SELECT * FROM b WHERE SOMEFUNC((1, 2), (3, 4))`: plan.NewProject(
		...
		plan.NewFilter(
			expression.NewUnresolvedFunction(
				...
				expression.NewTuple(
					expression.NewLiteral(int32(3), sql.Int32),
					expression.NewLiteral(int32(4), sql.Int32),
				),
			),
	...
	`SELECT CAST(-3 AS UNSIGNED) FROM foo`: plan.NewProject(
		[]sql.Expression{
			expression.NewConvert(expression.NewLiteral(int32(-3), sql.Int32), expression.ConvertToUnsigned),
		},
		plan.NewUnresolvedTable("foo", ""),
	),
	`SELECT 2 = 2 FROM foo`: plan.NewProject(
		[]sql.Expression{
			expression.NewEquals(expression.NewLiteral(int32(2), sql.Int32), expression.NewLiteral(int32(2), sql.Int32)),
		},
		plan.NewUnresolvedTable("foo", ""),
	),
	...
	`SELECT a, b FROM t ORDER BY 2, 1`: plan.NewSort(
		[]plan.SortField{
			{
				Column:       expression.NewLiteral(int32(2), sql.Int32),
				Order:        plan.Ascending,
				NullOrdering: plan.NullsFirst,
			},
			{
				Column:       expression.NewLiteral(int32(1), sql.Int32),
				Order:        plan.Ascending,
				NullOrdering: plan.NullsFirst,
			},
		},
	...
	`SELECT 1 + 1;`: plan.NewProject(
		[]sql.Expression{
			expression.NewPlus(expression.NewLiteral(int32(1), sql.Int32), expression.NewLiteral(int32(1), sql.Int32)),
		},
		plan.NewUnresolvedTable("dual", ""),
	),
	`SELECT 1 * (2 + 1);`: plan.NewProject(
		[]sql.Expression{
			expression.NewMult(expression.NewLiteral(int32(1), sql.Int32),
				expression.NewPlus(expression.NewLiteral(int32(2), sql.Int32), expression.NewLiteral(int32(1), sql.Int32))),
		},
		plan.NewUnresolvedTable("dual", ""),
	),
	`SELECT (0 - 1) * (1 | 1);`: plan.NewProject(
		[]sql.Expression{
			expression.NewMult(
				expression.NewMinus(expression.NewLiteral(int32(0), sql.Int32), expression.NewLiteral(int32(1), sql.Int32)),
				expression.NewBitOr(expression.NewLiteral(int32(1), sql.Int32), expression.NewLiteral(int32(1), sql.Int32)),
			),
		},
		plan.NewUnresolvedTable("dual", ""),
	),
	`SELECT (1 << 3) % (2 div 1);`: plan.NewProject(
		[]sql.Expression{
			expression.NewMod(
				expression.NewShiftLeft(expression.NewLiteral(int32(1), sql.Int32), expression.NewLiteral(int32(3), sql.Int32)),
				expression.NewIntDiv(expression.NewLiteral(int32(2), sql.Int32), expression.NewLiteral(int32(1), sql.Int32))),
		},
		plan.NewUnresolvedTable("dual", ""),
	),
	...
	`SELECT '1.0' + 2;`: plan.NewProject(
		[]sql.Expression{
			expression.NewPlus(
				expression.NewLiteral("1.0", sql.Text), expression.NewLiteral(int32(2), sql.Int32),
			),
		},
		plan.NewUnresolvedTable("dual", ""),
	),
	...
	`SET autocommit=1, foo="bar"`: plan.NewSet(
		plan.SetVariable{
			Name:  "autocommit",
			Value: expression.NewLiteral(int32(1), sql.Int32),
		},
		plan.SetVariable{
			Name:  "foo",
	...
	`SET @@session.autocommit=OFF, off="0"`: plan.NewSet(
		plan.SetVariable{
			Name:  "@@session.autocommit",
			Value: expression.NewLiteral(int32(0), sql.Int32),
		},
		plan.SetVariable{
			Name:  "off",
	...
	`SET SESSION NET_READ_TIMEOUT= 700, SESSION NET_WRITE_TIMEOUT= 700`: plan.NewSet(
		plan.SetVariable{
			...
			Value: expression.NewLiteral(int32(700), sql.Int32),
		},
		plan.SetVariable{
	...
	"SELECT CASE foo WHEN 1 THEN 'foo' WHEN 2 THEN 'bar' ELSE 'baz' END": plan.NewProject(
		[]sql.Expression{expression.NewCase(
			...
			[]expression.CaseBranch{
				{
					Cond:  expression.NewLiteral(int32(1), sql.Int32),
					Value: expression.NewLiteral("foo", sql.Text),
				},
				{
					Cond:  expression.NewLiteral(int32(2), sql.Int32),
					Value: expression.NewLiteral("bar", sql.Text),
				},
			},
